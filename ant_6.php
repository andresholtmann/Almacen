          ...
          <div class="table-responsive">
              <table id="myTable" class="table table-striped">
                  ...
                  <th></th>  
                    ...
                    <tbody>
						<?php   
                         $query = "SELECT id_fac,prov_fac,num_fac,unoh_fac,ser_fac,norec_fac,stat_fac,razo_fac,id_prov,n_prov,nit_prov,DATE_FORMAT(fecho_fac,'%d/%m/%Y') AS niceDate FROM fac_".$tutu." inner join prov_".$tutu." on prov_fac = id_prov where stat_fac not like '00' order by fecho_fac DESC limit 50";
                        $result = mysqli_query($mysqli, $query) or trigger_error("Query Failed! SQL: $query - Error: ". mysqli_error($mysqli), E_USER_ERROR);
                        if($result) {
                            $abc=0;
                            while($row = mysqli_fetch_assoc($result)) {
