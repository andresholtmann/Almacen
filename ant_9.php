...
if( isset($_GET['del'])  )
     {
        ...
        if ($mysqli->query($deleteada) === TRUE) {
            ...
            ?>
                <script language="Javascript">
                    window.location = "?alm=_9&sal=<?php echo "".htmlspecialchars($_GET['sal'])."";?>";
                </script>
         <?php             
        } else {
            ...
            {
                    $sql = "UPDATE fac_".$tutu." SET  stat_fac='1' WHERE id_fac like '".htmlspecialchars($_GET['sal'])."' ";    
        ...
        if( isset($_POST['ingresano']) )
             ...
             $check = $mysqli->query("SELECT id_mov,fac_mov,prod_mov FROM mov_".$tutu." WHERE fac_mov='$fac' and prod_mov='$prod'");
             $coun=$check->num_rows;     
                     ...
                     $add = "INSERT INTO mov_".$tutu." (fac_mov,tip_mov,prod_mov,cant_mov,preu_mov,useing) VALUES ('$_POST[fac_mov]','$_POST[tip_mov]','$_POST[prod_mov]','$_POST[cant_mov]','$_POST[preu_mov]','$ide')";
                ...
                ?>
                <script language="Javascript">
                    window.location = "?alm=_9&sal=<?php echo $pivo;?>";
                </script>
         <?php                    
             }
      ...
      <div class="row">
        <div class="col-sm-3 col-md-2 sidebar"><br>
                ...
                ?>
						...
						<?php   
                         $query = "SELECT id_pers,n_pers,ubi_pers,pue_pers,ext_pers,id_dep,n_dep FROM pers_".$tutu." inner join depto_".$tutu." on ubi_pers = id_dep where id_pers like '".htmlspecialchars($_GET['sal'])."'";
                        $result = mysqli_query($mysqli, $query) or trigger_error("Query Failed! SQL: $query - Error: ". mysqli_error($mysqli), E_USER_ERROR);
                        if($result) {
                            while($row = mysqli_fetch_assoc($result)) {
                         ?> 
                            <h4 class="text-info"><?php echo $row['n_pers'];?></h4>           
                            <hr>      
        		              <p><?php echo $row['n_dep'];?></p>
        		              <p><?php echo $row['pue_pers'];?></p>            
        		              <p>ext: <?php echo $row['ext_pers'];?></p>                        
						<?php
                            }
                        }
                        mysqli_free_result($result);?>              
  ...
  ?>     
            ...
            <div class="alert-message alert-message-info">
                <h4>Agregar Producto</h4>
                     <form class="form-inline" method="post" action="?alm=_9&sal=<?php echo "".htmlspecialchars($_GET['sal'])."";?>">
                      <div class="form-group">
                        <section id="intro" >      
                                <select name="prod_mov" class="form-control" >
                                    <?php                    
                                        $queryada = "SELECT id_pro,cod_pro,pro_pro FROM prod_".$tutu." order by cod_pro ASC ";
                      ...
                      <div class="form-group">
                        ...
                        <input type="hidden" name="fac_mov" value="<?php echo "".htmlspecialchars($_GET['sal']).""; ?>">                                                                          
          ...
          <div class="table-responsive">
              <table id="myTable" class="table table-striped table-bordered">
                  ...
                  <th>&nbsp;</th>
                    ...
                    <tbody>
						<?php   
                         $query = "SELECT id_mov,fac_mov,tip_mov,prod_mov,preu_mov,cant_mov,id_pro,cod_pro,pro_pro,pres_pro,id_pres,pres_pres FROM mov_".$tutu." inner join prod_".$tutu." on prod_mov = id_pro inner join pres_".$tutu." on pres_pro = id_pres where tip_mov like 'ENG' and fac_mov like '".htmlspecialchars($_GET['sal'])."' order by id_mov DESC";
                        $result = mysqli_query($mysqli, $query) or trigger_error("Query Failed! SQL: $query - Error: ". mysqli_error($mysqli), E_USER_ERROR);
                        $conti = 1 ;
                        $tota = 0 ;                        
                        if($result) {
                         ...
                         ?> 
                            ...
                            <td class="alimano">
                                ...
                                {                           
                                        ...
                                        {
                                        ?> 
                                            <a class="btn btn-danger btn-info-full" href="?alm=_9&del=<?php echo $row['id_mov'];?>&fac=<?php echo "".htmlspecialchars($_GET['sal'])."";?>" target="_self"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a>  
                                        <?php                        
                                        }
                                            else
                                        {
                    ...
                    <tfoot>
                            ...
                            <td class="alimano">
                                ...
                                {
                                ?> 
                                    <a class="btn btn-success btn-info-full" href="?alm=_9&acctt=true&fac=<?php echo "".htmlspecialchars($_GET['sal'])."";?>" target="_self"><span class="glyphicon glyphicon-ok-circle" aria-hidden="true"></span></a>                            
                                <?php                        
                                }
                                    else
                                {
