          ...
          <div class="table-responsive">
              <table id="myTable" class="table table-striped">
                  ...
                  <th>fecha</th> 
                    ...
                    <tbody>
						<?php   
                         $query = "SELECT id_kar,prov_kar,num_kar,stat_kar,fecho_kar,id_pers,n_pers,kar_desc,pue_pers,DATE_FORMAT(fecho_kar,'%d/%m/%Y') AS niceDate FROM kardex_".$tutu." inner join pers_".$tutu." on prov_kar = id_pers where stat_kar not like '3'  order by fecho_kar DESC limit 150";
                        $result = mysqli_query($mysqli, $query) or trigger_error("Query Failed! SQL: $query - Error: ". mysqli_error($mysqli), E_USER_ERROR);
                        if($result) {
                            while($row = mysqli_fetch_assoc($result)) {
                         ?> 
