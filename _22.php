...
if( isset($_GET['upq'])  )
     {
        $sql = "UPDATE kardex_".$tutu." SET stat_kar='99' WHERE id_kar like '".htmlspecialchars($_GET['elu'])."' ";    
        if ($mysqli->query($sql) === TRUE) {
        echo "Kardex - ";
                }                 
    
        $sql = "UPDATE movprob_".$tutu." SET st_movprob='99' WHERE kar_movprob like '".htmlspecialchars($_GET['elu'])."' ";    
        if ($mysqli->query($sql) === TRUE) {
        echo "Movimientos - ";
                }                 

        $sql = "UPDATE mov_".$tutu." SET st_mov='99' WHERE kar_mov like '".htmlspecialchars($_GET['elu'])."' ";    
        if ($mysqli->query($sql) === TRUE) {
                echo "Entregados";
                }                 
    //        exit();
...
<div class="container-fluid">   
      <div class="row">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h2 class="cabece">Inhabilitar salida</h2>
            <?php   
                 $query = "SELECT id_kar,prov_kar,num_kar,stat_kar,fecho_kar,stat_kar,id_pers,n_pers,DATE_FORMAT(fecho_kar,'%d/%m/%Y') AS niceDate FROM kardex_".$tutu." inner join pers_".$tutu." on prov_kar = id_pers  where id_kar like '".htmlspecialchars($_GET['del'])."'";
          ...
          <div class="table-responsive">
              <table id="myTable" class="table table-striped table-bordered">
                  ...
                  <th>&nbsp;</th>
                    ...
                    <tbody>
<?php   
                         $query = "SELECT id_movprob,kar_movprob,tip_movprob,prod_movprob,preu_movprob,cant_movprob,id_pro,cod_pro,pro_pro,pres_pro,id_pres,pres_pres FROM movprob_".$tutu." inner join prod_".$tutu." on prod_movprob = id_pro inner join pres_".$tutu." on pres_pro = id_pres where tip_movprob like 'EGR' and kar_movprob like '".htmlspecialchars($_GET['del'])."' order by id_movprob DESC";
                        $result = mysqli_query($mysqli, $query) or trigger_error("Query Failed! SQL: $query - Error: ". mysqli_error($mysqli), E_USER_ERROR);
                        $conti = 1 ;
                        $tota = 0 ;                        
                        if($result) {
...
?>                                 
            ...
            <br><br>
            <a href="?alm=_22&elu=<?php echo htmlspecialchars($_GET['del']) ; ?>&upq=true" type="button" class="btn btn-danger btn-block">Eliminar salida</a>   		                      
