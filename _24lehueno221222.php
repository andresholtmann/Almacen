...
<?php  
 $query = "SELECT id_mov,fac_mov,kar_mov,tip_mov,prod_mov,cant_mov,preu_mov,st_mov,DATE_FORMAT(fech_mov,'%d/%m/%Y') AS niceDate,DATE_FORMAT(fech_mov,'%Y') AS movo FROM mov_".$tutu." where prod_mov like '".$ale."' and st_mov like '1' and fech_mov between '2019-01-01' and '2022-12-31' order by fech_mov,id_mov ASC";
    $result = mysqli_query($mysqli, $query) or trigger_error("Query Failed! SQL: $query - Error: ". mysqli_error($mysqli), E_USER_ERROR);
    $tota = 0 ;
    $toto = 0 ;
    $pagi = 1 ;
